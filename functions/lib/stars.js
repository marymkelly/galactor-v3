...
async function getStars(ra = 28.4, dec = -80.6, opts) {
  ...
  try {
    const response = await axios.get(getStarsUrl);
    const parsedData = await xml2js.parseStringPromise(response.data, {trim: true});
    ...
    const validateStars = parsedData.response.star.map(async (star) => {
      ...
      try {
        const response = await axios.get(validateStarsUrl);
        const parsedData = await xml2js.parseStringPromise(response.data, {trim: true});
